...
def argparser():
    ...
    parser.add_argument(
        "--config", "-c", type=str, default="config/test.yaml", help="config file"
    )
    parser.add_argument(
        "--ckpt_path", "-p", type=str, default="model.ckpt", help="model checkpoint"
    )
...
def test(config_path: Union[str, Path], ckpt_path: Union[str, Path]):
    ...
    with initialize(config_path=str(config_path.parent), version_base="1.1"):
        ...
        testing_pipline(config, str(ckpt_path))
